(() => {
  ...
  function addOperationsViews() {
    ...
    show(`${header('GuardianX Tactical Map','Live emergency response dashboard')}
    <div class="glass-panel rounded-xl p-4">
        <div id="world-map" class="w-full h-[620px] rounded-lg overflow-hidden"></div>

        <div class="flex gap-6 mt-4 text-sm">
            <span class="text-error">🚨 Emergency</span>
            <span class="text-green-400">🏥 Hospital</span>
            <span class="text-orange-400">🚒 Fire Station</span>
            <span class="text-blue-400">👮 Police Station</span>
            <span class="text-primary">📍 Responder</span>
        </div>
    </div>`);

    await loadLeaflet();

    const map = L.map('world-map').setView([17.4030,78.4910],15);
    setTimeout(() => {
    map.invalidateSize();
}, 300);
    ...
    const incident=[17.4030,78.4910];

L.marker(incident)
    .addTo(map)
    .bindPopup("🚨 Emergency Incident")
    .openPopup();

const query = `
[out:json];
(
  node["amenity"="hospital"](around:5000,17.4030,78.4910);
  way["amenity"="hospital"](around:5000,17.4030,78.4910);

  node["amenity"="police"](around:5000,17.4030,78.4910);
  way["amenity"="police"](around:5000,17.4030,78.4910);

  node["amenity"="fire_station"](around:5000,17.4030,78.4910);
  way["amenity"="fire_station"](around:5000,17.4030,78.4910);
);
out center;
`;
...
.then(data => {
    ...
    console.log("Total Places:", data.elements.length);
    ...
    const incidentLat = 17.4030;
    const incidentLon = 78.4910;

    function showNearest(list, count, emoji, icon) {
        ...
        list.sort((a, b) => {
            ...
            return distance(alat, alon, incidentLat, incidentLon) -
                   distance(blat, blon, incidentLat, incidentLon);

        });

        list.slice(0, count).forEach(place => {
            ...
            console.log("Adding:", place.tags?.amenity, place.tags?.name);

            L.marker([lat, lon], { icon })
                .addTo(map)
                .bindPopup(`<b>${emoji} ${place.tags?.name || "Unknown"}</b>`);
        });
    }

  console.log("Hospitals found:", hospitals.length);
  console.log("Police found:", policeStations.length);
  console.log("Fire found:", fireStations.length);

  showNearest(hospitals, 1, "🏥", hospitalIcon);
  showNearest(policeStations, 1, "👮", policeIcon);
  showNearest(fireStations, 1, "🚒", fireIcon);
...
.catch(err => console.error(err));
    ...
    if(navigator.geolocation){
        ...
        navigator.geolocation.getCurrentPosition(function(pos){
            ...
            L.polyline([user,incident],{
                color:"red",
                weight:4
            }).addTo(map);

            map.fitBounds([user,incident],{
                padding:[40,40]
            });
